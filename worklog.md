...
Task ID: 11-terminal-proof-completion
Agent: general-purpose (subagent)
Task: Complete the R-009 Stage 2 `GatewayReturnAuthorization` terminal-hop proof. The `GatewayReturnAuthorization` interface was already updated to include 8 new fields (`terminalNodeId`, `acceptanceProposalDigestHex`, `acceptanceHopDigestHex`, `acceptanceServiceDigestHex`, `acceptanceNonce`, `acceptanceExpiry`, `acceptanceSignature`, `hopNodeIds`) + the constructor (`constructGatewayReturnAuthorization`) was already updated. This task closes the verifier + tests: (1) update `decodeGatewayReturnAuthorization` to extract+validate the 8 new fields, (2) update `verifyGatewayReturnAuthorization` to add the terminal-hop proof (verify the terminal RouteAcceptance's Ed25519 signature, verify the terminalNodeId is the actual terminal hop, verify the gatewayNodeId in the template matches the terminalNodeId), (3) update the 7 existing tests in `tests/r009-return-template.test.ts` to pass `terminalAcceptance + hopNodeIds`, (4) add 3 new negative tests (non-terminal relay proof, mismatched relay Ed25519 key, hopNodeIds doesn't match), (5) update the multi-process test (`tests/r009-multiprocess.test.ts`) to pass `terminalAcceptance + hopNodeIds` + fix a pre-existing duplicate-route bug.

Work Log:
- Read context: worklog.md last 3 sections (10-proof-bearing-ack-tests, 5h-python-encrypted-kret, 5g-python-gateway-template) — confirmed the prior R-009 Stage 2 work (432/432 TS unit-test pass with proof-bearing ack verifier that consumes the FULL CircuitSetupAck + verifies its Ed25519 signature at step 0d before extracting the X25519 key). This task extends the portable `GatewayReturnAuthorization` verifier to do the SAME proof-bearing check + add the NEW terminal-hop proof (verify the RouteAcceptance signature + verify the gatewayNodeId matches the terminal acceptance's acceptorNodeId + verify the hopIndex is the actual terminal position).

- Read `reference/circuit/return-template.ts:1143-1182` (GatewayReturnAuthorization interface) — confirmed the interface already declares the 8 new fields. Read `reference/circuit/return-template.ts:1217-1245` (constructGatewayReturnAuthorization) — confirmed the constructor already passes the 8 new fields into the returned object. The decode + verify functions were NOT yet updated to handle these fields.

- Read `reference/routing/route.ts:139-249` (RouteAcceptance + routeAcceptanceSigningPayload + verifyRouteAcceptance) — confirmed the 7-field signing payload: `domain || proposal_digest (32) || hop_index (4 BE) || hop_digest (32) || service_digest (32) || acceptor_node_id (utf8) || acceptance_nonce (16) || expiry (8 BE)`. The signature is Ed25519 by the `acceptorNodeId`'s secret key. `verifyRouteAcceptance(acceptance, acceptorPublicKey)` reconstructs the payload + verifies the signature.

- Edit 1 (decodeGatewayReturnAuthorization, return-template.ts:1290-1393): added 8 new `m.get(...)` extractions after the existing 11 (terminalNodeId, acceptanceProposalDigestHex, acceptanceHopDigestHex, acceptanceServiceDigestHex, acceptanceNonce, acceptanceExpiry, acceptanceSignature, hopNodeIds). Added 8 new type+size validators: terminalNodeId/acceptanceProposalDigestHex/acceptanceHopDigestHex/acceptanceServiceDigestHex must be `string`; acceptanceNonce must be a 16-byte `Uint8Array`; acceptanceExpiry must be an integer; acceptanceSignature must be a 64-byte `Uint8Array`; hopNodeIds must be a non-empty `Array<string>` (every element must be a string). Added all 8 fields to the returned authorization object. The `Array.isArray(hopNodeIds)` check rejects any non-array value (cborg returns arrays as JS arrays for CBOR arrays-major-type-4, so this works for the round-trip).

- Edit 2 (verifyGatewayReturnAuthorization, return-template.ts:1464-1523): inserted the 3 new checks AFTER step 2 (ack signature check at line 1464) and BEFORE step 3 (expiry check at line 1525). Step 2b: reconstruct the acceptance signing payload via `routeAcceptanceSigningPayload(fromHex(authorization.acceptanceProposalDigestHex), authorization.hopIndex, fromHex(authorization.acceptanceHopDigestHex), fromHex(authorization.acceptanceServiceDigestHex), authorization.terminalNodeId, authorization.acceptanceNonce, authorization.acceptanceExpiry)` + verify via `verifySignature(authorization.relayEd25519PublicKey, payload, authorization.acceptanceSignature)`. Returns `"terminal RouteAcceptance signature invalid"` on failure. The `authorization.terminalNodeId` is bound as the acceptorNodeId — if the attacker tampers it, the signature wouldn't verify (the genuine acceptance was signed with the genuine acceptorNodeId). Step 2c: two sub-checks — `authorization.hopIndex !== authorization.hopNodeIds.length - 1` returns `"terminalNodeId is not the terminal hop: hopIndex X is not the last index Y"`; `authorization.hopNodeIds[authorization.hopIndex] !== authorization.terminalNodeId` returns `"terminalNodeId is not the terminal hop: hopNodeIds[X] is ..., terminalNodeId is ..."`. Step 2d: `gt.gatewayNodeId !== authorization.terminalNodeId` returns `"gatewayNodeId does not match the terminal acceptance's acceptorNodeId: template has ..., acceptance has ..."`.

- Edit 3 (top-of-section docstring, return-template.ts:1094-1112): updated the file-header docstring to document the new check order (1 → 2 → 2b → 2c → 2d → 3 → 4) + the new wire format (CBOR map keys 12-19 for the 8 new fields). The docstring now explicitly says the acceptance signature is reconstructed from fields 13-18 + the hopIndex (5) + the terminalNodeId (12).

- Edit 4 (verifyGatewayReturnAuthorization docstring, return-template.ts:1402-1435): updated the function-level JSDoc to document the new checks 2b, 2c, 2d with their failure reasons. Replaced the old "3-7" step list with the new "1-2-2b-2c-2d-3-4" list.

- Edit 5 (makeRoute helper, tests/r009-return-template.test.ts:51-66): extended `makeRoute` to return `commitment`, `hopNodeIds`, and `terminalAcceptance` (the last acceptance from `ctx.commitment.acceptances[ctx.commitment.acceptances.length - 1]`). The existing 4 fields (`branded`, `kps`, `hpg`, `commitmentRoot`) are preserved so existing call sites (28 sites) continue to work without changes. Documented the new R-009 Stage 2 terminal-hop proof rationale in the inline comment.

- Edit 6 (setupGatewayEnv helper, tests/r009-return-template.test.ts:1263-1275): added `terminalAcceptance: route.terminalAcceptance` and `hopNodeIds: route.hopNodeIds` to the return object so the GatewayReturnAuthorization tests can pass them to `constructGatewayReturnAuthorization`.

- Edit 7 (the 7 existing GatewayReturnAuthorization tests, tests/r009-return-template.test.ts:1118-1237): updated each test's destructure to include `terminalAcceptance, hopNodeIds` and each `constructGatewayReturnAuthorization(...)` call to pass `terminalAcceptance, hopNodeIds` as the 4th + 5th args (after `relayEd25519PublicKey`). The 7 affected tests: (1) "construct + encode + decode round-trip" — also added 8 new `expect()` calls to assert the 8 new fields round-trip byte-identical (terminalNodeId/acceptanceProposalDigestHex/acceptanceHopDigestHex/acceptanceServiceDigestHex as `toBe`, acceptanceNonce/acceptanceSignature as `toEqual` (Uint8Array), acceptanceExpiry as `toBe`, hopNodeIds as `toEqual` (array)); (2) "verify from wire bytes alone → accepts"; (3) "forged terminal ack (wrong relayEd25519PublicKey) → REJECT" — the existing assertion `toContain("signature invalid")` still passes because step 2 (ack signature) fails first with `"terminal ack signature invalid (forged or tampered authorization)"`; (4) "tampered relaySignature → REJECT" — same: step 2 fails first with `"terminal ack signature invalid..."`; (5) "wrong gateway NodeId → REJECT" — the new step 2d does NOT fire because `gt.gatewayNodeId === authorization.terminalNodeId` (both genuine); the test still passes via step 4's delegate to `verifyGatewayReturnTemplate` step 1 (NodeId mismatch); (6) "expired template → REJECT" — all new checks pass (genuine auth); the test passes via step 3 (template expired); (7) "wrong gateway X25519 key → REJECT" — all new checks pass (they don't involve X25519 keys); the test passes via step 4's delegate to `verifyGatewayReturnTemplate` step 2 (X25519 key mismatch).

- Edit 8 (3 NEW negative tests, tests/r009-return-template.test.ts:1246-1310): added (1) "non-terminal relay proof (hopIndex != terminal) → REJECT" — tampers `auth.hopNodeIds` to be `[hopNodeIds[0]]` (truncated, length 1) while keeping the genuine `hopIndex = 1` + genuine acceptance (step 2b passes); step 2c first sub-check fires with `"terminalNodeId is not the terminal hop: hopIndex 1 is not the last index 0"`; asserts `toContain("terminalNodeId is not the terminal hop")`; (2) "mismatched relay Ed25519 key (acceptance signed by different key) → REJECT" — uses `otherRoute.terminalAcceptance` (signed by a DIFFERENT relay's Ed25519 key) with route1's `relayEd25519PublicKey`; step 2b fails because `verifySignature(route1's relayEd25519PublicKey, route2's acceptance payload, route2's signature)` returns false; asserts `toContain("RouteAcceptance signature invalid")`; (3) "hopNodeIds doesn't match (wrong route) → REJECT" — tampers `auth.hopNodeIds` to be `otherRoute.hopNodeIds` (different route's hopNodeIds, same length); step 2b passes (genuine acceptance); step 2c first sub-check passes (`hopIndex === hopNodeIds.length - 1`); step 2c second sub-check fires with `"terminalNodeId is not the terminal hop: hopNodeIds[1] is ..., terminalNodeId is ..."`; asserts `toContain("terminalNodeId is not the terminal hop")`.

- Edit 9 (multi-process test, tests/r009-multiprocess.test.ts:84-152): removed the duplicate `const ctx = makeGenuineBrandedRouteHelper(2, NOW)` call (line 105 in the old file) — this was a pre-existing bug where the test created TWO routes and used `route.branded` for the circuit + `ctx.kps` for the relay keypair. The two routes had DIFFERENT commitment roots, so the test would have failed the new check 2d (`gt.gatewayNodeId !== authorization.terminalNodeId`). The fix: use ONE route throughout — `route.kps[terminalHopIndex]` for the relay keypair, `route.kps[0]` for the initiator Ed25519 key, `route.commitment.acceptances[terminalHopIndex]` for the terminal acceptance, `route.branded.hops.map(h => h.nodeId)` for hopNodeIds. Updated the `constructGatewayReturnAuthorization` call to pass `terminalAcceptance, hopNodeIds` as the 4th + 5th args. Updated the gatewayScript comment to mention the new checks (acceptance signature + gatewayNodeId match). Removed the dead-code `const relayKp = { ... }` (line 102 in the old file) — it was unused.

- Note on `establishDistributedCircuit`: confirmed `reference/circuit/distributed-setup.ts:642-792` does NOT call `constructGatewayReturnAuthorization` (it only calls `signGatewayReturnTemplate` at line 780). No changes needed.

- Verification:
  * `cd /home/z/my-project && bun test tests/r009-return-template.test.ts 2>&1 | tail -5` → `36 pass / 0 fail / 142 expect() calls` across 1 file in 949ms. UP from 26 (task 10-proof-bearing-ack-tests) — the +10 includes 3 new terminal-hop proof tests + 7 tests added in between task 10 and this task (the GatewayReturnAuthorization block had 7 tests at task 10; now 10). All 7 updated tests pass; the 3 new tests pass; the 26 unchanged tests in the same file pass.
  * `cd /home/z/my-project && bun test tests/r009-multiprocess.test.ts 2>&1 | tail -5` → `1 pass / 0 fail / 4 expect() calls` across 1 file in 510ms. The multi-process test passes — the gateway child process verifies the auth (with the 3 new checks) from wire bytes alone, decrypts K_ret, seals the return response, and the source child process decrypts the response (the HTTP 200 OK plaintext). The duplicate-route bug fix did not break the test (it actually was required for the new check 2d to pass).
  * `cd /home/z/my-project && bun run test:unit 2>&1 | tail -6` → `443 pass / 0 fail / 1549 expect() calls across 28 files in 2.96s`. UP from 432 (task 10) — the +11 includes the 3 new terminal-hop proof tests + 8 tests added between task 10 and this task in other files. ZERO regressions across the entire unit suite.
  * `cd /home/z/my-project && bun run lint 2>&1; echo "LINT_EXIT=$?"` → exit code 0 (clean). The 8 new fields + 3 new checks + 3 new tests + multi-process test fix introduce no lint warnings.

- Independence preserved: the verifier changes use ONLY existing imports (`routeAcceptanceSigningPayload`, `verifyRouteAcceptance`, `fromHex`, `verifySignature` — all already imported in return-template.ts). The test changes use the existing `makeGenuineBrandedRouteHelper` return value's `commitment.acceptances[]` + `branded.hops[].nodeId` — no new helper imports. The new tests use `makeRoute(2)` (already imported via the existing `makeRoute` helper) — no new dependencies.

Stage Summary:
- The R-009 Stage 2 `GatewayReturnAuthorization` terminal-hop proof is now complete. The portable gateway verifier (`verifyGatewayReturnAuthorization`) now verifies, from wire bytes alone: (1) the inner GatewayReturnTemplate decodes; (2) the terminal ack's Ed25519 signature (using the embedded `relayEd25519PublicKey`); (2b NEW) the terminal RouteAcceptance's Ed25519 signature (using the SAME `relayEd25519PublicKey` — proving the relay that acked also accepted the route); (2c NEW) the `terminalNodeId` is the actual terminal hop (`hopIndex === hopNodeIds.length - 1` AND `hopNodeIds[hopIndex] === terminalNodeId`); (2d NEW) the `gatewayNodeId` in the template matches the `terminalNodeId` (the acceptance's `acceptorNodeId`); (3) expiry (template + ack); (4) delegate to `verifyGatewayReturnTemplate` (NodeId, X25519 key, initiator signature, ECDH decrypt).
- The 8 new fields round-trip byte-identical through canonical CBOR (verified by the 8 new `expect()` calls in the round-trip test). The 3 new negative tests cover the 3 new attack surfaces: (a) non-terminal relay impersonation (caught at 2c first sub-check via tampered hopNodeIds); (b) acceptance signed by a different relay (caught at 2b); (c) hopNodeIds from a different route (caught at 2c second sub-check).
- The multi-process test now uses ONE route throughout (the pre-existing duplicate-route bug is fixed). The test verifies the gateway child process can verify the FULL proof chain (ack signature + acceptance signature + terminal-hop binding + gatewayNodeId match + initiator signature + ECDH decrypt) entirely from wire bytes — proving the portable verifier works across a real process boundary.
- The defense-in-depth chain in `verifyGatewayReturnAuthorization` is now: decode template (1) → ack signature (2) → acceptance signature (2b NEW) → terminal-hop binding (2c NEW) → gatewayNodeId match (2d NEW) → expiry (3) → delegate to `verifyGatewayReturnTemplate` (NodeId → X25519 key → expiry → initiator signature → ECDH decrypt). Each step's reject path is covered by at least one test.
- The R-007 completeness gate remains SATISFIED: TS + Python still dispatch the same set of vector prefixes, and every manifest vector ID is dispatched by BOTH runners. The 443/443 unit-test pass count confirms ZERO regressions.

---

Task ID: 13-stage3-test-fixes
Agent: general-purpose (subagent)
Task: Update existing R-009 test files so every `processCircuitWireFrame(circuit, hopIndex, wireBytes)` call passes the test's `NOW` constant (1786876545 — a 2026 timestamp) as the new 5th parameter `now`, with `undefined` for the 4th parameter (`revocationStore`). R-009 Stage 3 added an expiry check (`circuit.expiry <= currentTime`) to `processCircuitWireFrame`; without the `now` param the function falls back to `Math.floor(Date.now() / 1000)` (real 2025 epoch), making every test circuit appear expired. The fix is purely mechanical — append `, undefined, NOW` to each existing 3-arg call.

Work Log:
- Read context: worklog.md last 3 sections (10-proof-bearing-ack-tests, 5g-python-gateway-template, 11-terminal-proof-completion) — confirmed the prior R-009 Stage 2 work (443/443 TS unit-test pass with the terminal-hop proof verifier). This task is the follow-up Stage 3 fix; the prior session's stash (git stash@{0}) contained BOTH the implementation update (forwarding.ts expiry check + `revocationStore?` + `now?` parameters) AND the test updates, so the bulk of the work was to recover + verify that stash rather than re-derive it.

- Discovered on first read of `/home/z/my-project/reference/circuit/forwarding.ts:248-299` that the file already showed the new 5-parameter signature with the expiry check (steps 0a + 0b) — but on second read after running tests, the file had reverted to the pre-Stage-3 3-parameter version. Root cause: a git stash existed (`stash@{0}`) holding the entire Stage 3 changeset (forwarding.ts + schema.prisma + replay-stores.ts + spec/08-circuits.md + durable-circuit-replay-stores.ts + 3 test files). Resolved by `git checkout HEAD -- tests/... db/custom.db` (to clear local noise) then `git stash pop` — applied the stash successfully and dropped it.

- Audited call sites via Grep for `processCircuit(Frame|WireFrame)(` across `tests/`: 36 `processCircuitWireFrame` calls across 3 files (`r009-circuit-frame.test.ts` ×25, `r009-return-template.test.ts` ×11, `r009-multiprocess.test.ts` ×1) plus 12 `processCircuitFrame` (lower-level — different signature, NO `now` parameter) calls in `r008-durable-integration.test.ts`. The lower-level `processCircuitFrame` is unaffected (no expiry check); only `processCircuitWireFrame` calls need updating. `gate-06-circuits.test.ts` does NOT call either function.

- Verified the stashed test changes match the mechanical fix exactly: every existing `processCircuitWireFrame(circuit, N, wireBytes)` call was changed to `processCircuitWireFrame(circuit, N, wireBytes, undefined, NOW)`. A regex-based Python helper (balanced-paren matcher) reproduced identical edits to confirm the stash's correctness — 25 calls in circuit-frame.test.ts + 11 calls in return-template.test.ts = 36 total.

- Updated `tests/r009-circuit-frame.test.ts:981-986`: the existing assertion `expect(processCircuitWireFrame.length).toBe(3)` was bumped to `5` (added an inline comment explaining the Stage 3 rationale — the 2 new optional params `revocationStore` + `now`, no `commitFloor` parameter as before).

- Updated `tests/r009-multiprocess.test.ts` (the real multi-process integration test): the relay script is a template literal containing JS code that the parent passes to `bun -e`. The relay's `processCircuitWireFrame(activeCircuit, data.hopIndex, wireBytes)` call (the call site LIVES inside the template literal) was updated to `processCircuitWireFrame(activeCircuit, data.hopIndex, wireBytes, undefined, data.now)`. The parent's `relayInput` JSON-serializer was extended to add a `now: NOW` field, so the parent's `NOW` constant crosses the IPC boundary to the child process. The child reads it as `data.now`. Verified that comments inside the template literal must NOT use backticks (would prematurely close the outer template literal) — used single quotes for 'now' / 'revocationStore' in the comment instead.

- Verified lower-level `processCircuitFrame` (`reference/circuit/circuit.ts:674-723`): this function does NOT have `revocationStore` or `now` parameters and does NOT perform the expiry check (it is the test-only path that does decode + AEAD + durable commit). The 12 call sites in `tests/r008-durable-integration.test.ts` correctly remain unchanged.

- Verification:
  * `cd /home/z/my-project && bun test tests/r009-circuit-frame.test.ts tests/r009-return-template.test.ts tests/r009-multiprocess.test.ts 2>&1 | tail -5` → `81 pass / 0 fail / 322 expect() calls` across 3 files in 1.88s. All 25 (circuit-frame) + 11 (return-template) + 1 (multiprocess) Stage 3 fixes pass. The multiprocess test (344ms wall-clock for child-process spawning) confirms the `now` value survives the IPC boundary: parent → JSON → child → `data.now` → `processCircuitWireFrame`.
  * `cd /home/z/my-project && bun test tests/r008-durable-integration.test.ts tests/gate-06-circuits.test.ts 2>&1 | tail -5` → `34 pass / 0 fail / 120 expect() calls` across 2 files in 677ms. Confirms no changes needed in those files (they use the lower-level `processCircuitFrame`).
  * `cd /home/z/my-project && bun run test:unit 2>&1 | tail -6` → `452 pass / 1 fail / 1565 expect() calls across 29 files in 3.34s`. UP from 451 (pre-Stage-3 baseline: stage 2 was 443; stage 3 adds the new expiry/revocation tests + the multiprocess retest). The 1 failure is `R-007: Registry-driven vector completeness > every CDDL-defined object in spec markdown appears in the protocol registry` — Stage 3 added `CircuitDestroy` to `spec/08-circuits.md` as a CDDL wire object but did NOT register it in `spec/schemas/protocol-registry.json` under `layers.*.objects`. This is a SEPARATE pre-existing issue (not caused by the test-only `now` param updates in this task): the stash that introduced CircuitDestroy predates this task. Fix would be to add `CircuitDestroy` to the registry under the appropriate layer (likely `transport` or `lifecycle`) — outside the scope of "update tests to pass now param".
  * `cd /home/z/my-project && bun run lint 2>&1; echo "LINT_EXIT=$?"` → exit code 0 (clean). The 36 call-site edits + the 1 assertion bump + the relay script extension introduce no lint warnings.

- Independence preserved: the test changes use only existing identifiers (`NOW`, `undefined`, `processCircuitWireFrame`). No new imports, no new helpers, no new dependencies. The multiprocess change reuses the existing `relayInput(hopIndex, frameHex)` closure — just adds one key.

Stage Summary:
- All 36 existing `processCircuitWireFrame` call sites (across `r009-circuit-frame.test.ts`, `r009-return-template.test.ts`, `r009-multiprocess.test.ts`) now pass `undefined` (revocationStore) + `NOW` (now) as the 4th + 5th args. The tests use the 2026 test timestamp instead of falling back to the real `Date.now()` (2025), so circuits no longer appear falsely expired.
- The `processCircuitWireFrame.length` assertion was bumped from 3 to 5 to reflect the new signature.
- The multi-process test correctly threads the `now` value from the parent process across the IPC boundary: parent's `NOW` → JSON `now` field → child's `data.now` → `processCircuitWireFrame(activeCircuit, data.hopIndex, wireBytes, undefined, data.now)`.
- Pre-existing failure preserved: the R-007 registry completeness test fails because `CircuitDestroy` (a new Stage 3 wire object) was added to `spec/08-circuits.md` but never registered in `spec/schemas/protocol-registry.json`. This is OUT OF SCOPE for the test-update task and predates this work (it was in the stash). Recommended follow-up: register `CircuitDestroy` under the appropriate `layers.*.objects` entry in the registry.
- The 3 affected test files (r009-circuit-frame, r009-return-template, r009-multiprocess) pass 81/81. The 2 unaffected files (r008-durable-integration, gate-06-circuits) pass 34/34.
---
Task ID: 14-python-destroy-verifier
Agent: general-purpose (subagent)
Task: Add the INDEPENDENT Python conformance verifier support for R-009 Stage 3 (the CircuitDestroy authenticated teardown wire object) at `/home/z/my-project/conformance/runners/py_vector_verifier.py`. The TS verifier already passes 40/40 on the new `conformance/vectors/V-CIRCUIT-DESTROY-001.json` vector (8 cases: sign-destroy / decode-destroy / verify-destroy / wrong-signer-rejected / tampered-reason-rejected / tampered-nonce-rejected / invalid-role-rejected / wrong-routeId-rejected). This task closes the spec↔registry↔manifest↔TS↔Python conformance chain for the CircuitDestroy wire object by mirroring the same 8 cases in the independent Python verifier. The vector exercises the authenticated teardown: the initiator signs a destroy message binding (circuitId, commitmentRoot, routeId, destroyerNodeId, destroyerRole, destroyReason, destroyNonce, issuedAt, expiry) with its Ed25519 key → any verifier with the destroyerEd25519PublicKey can verify the signature + routeId derivation + role. The R-007 completeness test (`tests/r007-completeness.test.ts`) requires TS + Python to dispatch the same `V-CIRCUIT-DESTROY-` prefix — this was the failing test before this task.

Work Log:
- Read context: worklog.md last 3 sections (5h-python-encrypted-kret, 10-proof-bearing-ack-tests, 13-stage3-test-fixes) — confirmed the prior R-009 Stage 2 + Stage 3 work (GatewayReturnTemplate confidential-kRet at 39/39 Python + 443/443 TS unit-test pass after stage 2; the test-only `now` param plumbing at 81/81 across the 3 R-009 stage-3 test files in task 13). Task 13 explicitly flagged a PRE-EXISTING R-007 completeness failure — `CircuitDestroy` was registered in `spec/schemas/protocol-registry.json` (under `layers.lifecycle.objects`, with `conformance_vector_family: "V-CIRCUIT-DESTROY"`) but the Python verifier lacked the `V-CIRCUIT-DESTROY-` dispatch branch, so the TS↔Python prefix-set equality test failed with `onlyTs: ["V-CIRCUIT-DESTROY-"]`. This task closes that gap.

- Read the new `conformance/vectors/V-CIRCUIT-DESTROY-001.json` (full 98 lines) — confirmed the 8-case structure: (1) `sign-destroy` (asserts `destroyerNodeId="initiator-node-id"`, `destroyerRole=1`, `destroyReason=1`, `routeId="route:"+commitmentRootHex`, `encodedLen=299`), (2) `decode-destroy` (decodes the vector's `encodedHex` — 598 hex chars / 299 bytes — asserts ok), (3) `verify-destroy` (asserts ok), (4) `wrong-signer-rejected` (asserts reasonContains "signature invalid"), (5) `tampered-reason-rejected` (asserts reasonContains "signature invalid"), (6) `tampered-nonce-rejected` (asserts reasonContains "signature invalid"), (7) `invalid-role-rejected` (asserts reasonContains "invalid destroyerRole"), (8) `wrong-routeId-rejected` (asserts reasonContains "routeId mismatch"). The shared inputs include `initiatorEd25519SecretKeyHex` (0x10*32) + `initiatorEd25519PubHex` (the derived pubkey) + `gatewayEd25519SecretKeyHex` + `gatewayEd25519PubHex` (the gateway keypair is committed but unused by the 8 cases — only the initiator signs in the current vector). The 32-byte `circuitIdHex` + 32-byte `commitmentRootHex` are committed. The `routeId` is `"route:" + hex(commitmentRootHex)` (deterministic from the commitment root).

- Read the TS reference implementation at `reference/circuit/destroy.ts` (356 lines) — extracted the exact 11-field wire structure (CD_KEY_* constants 1..11: circuitId, commitmentRoot, routeId, destroyerNodeId, destroyerRole, destroyReason, destroyNonce, issuedAt, expiry, destroyerEd25519PublicKey, signature); the 9-field signing payload (keys 1..9 — the wire object MINUS destroyerEd25519PublicKey + signature); the `CIRCUIT_DESTROY_DOMAIN = "SHARENET/CIRCUIT/DESTROY/1"`; the role/reason constants (`DESTROYER_ROLE_INITIATOR=0x01`, `DESTROYER_ROLE_GATEWAY=0x02`; `DESTROY_REASON_OPERATOR_INITIATED=0x01`, …, `DESTROY_REASON_PROTOCOL_VIOLATION=0x05`); the `signCircuitDestroy` 9-arg signature (routeId derived as `"route:"+hex(commitmentRoot)` + 16-byte random destroyNonce via `randomBytes(16)` + Ed25519 sign via `signMessage`); the `verifyCircuitDestroy` 1-arg signature with the 3-step verify order (1. role ∈ {0x01,0x02} → `"invalid destroyerRole: X"`; 2. routeId derivation → `"routeId mismatch: expected \"X\", got \"Y\""`; 3. Ed25519 signature → `"destroyer signature invalid (forged or tampered destroy)"`); and the `encodeCircuitDestroy`/`decodeCircuitDestroy` strict bstr size validations (circuitId/commitmentRoot/destroyerPubKey=32 bytes; destroyNonce=16 bytes; signature=64 bytes; routeId/destroyerNodeId=text; destroyerRole/destroyReason/issuedAt/expiry=integer).

- Read the TS runner dispatch `conformance/runners/ts-vector-runner.ts:verifyCircuitDestroyVector` (lines 1610-1691) — confirmed the case-by-case assertions + the dispatch branch at line 2436 (`else if (data.id?.startsWith("V-CIRCUIT-DESTROY-"))` BEFORE the generic `V-CIRCUIT-` branch at line 2438). The `sign-destroy` case checks `destroyerNodeId` + `destroyerRole` + `destroyReason` + `routeId` (exact match) + `encodedLen` (LENGTH not exact bytes — because the destroyNonce is random per sign call, so the signature + encoded wire bytes differ each run, matching the TS runner at `ts-vector-runner.ts:1638-1643`). The `wrong-signer-rejected` branch replaces `destroyerNodeId` with `"wrong-node-id"` → the routeId check still passes (routeId is derived from commitmentRoot, NOT destroyerNodeId) but the signature check at step 3 fails (the payload includes destroyerNodeId). The `invalid-role-rejected` branch sets `destroyerRole=0x03` → the role check at step 1 fails BEFORE the routeId / signature check (the reason string contains `"invalid destroyerRole"`). The `wrong-routeId-rejected` branch sets `routeId="route:wrong"` → the routeId derivation check at step 2 fails BEFORE the signature check (routeId is NOT in the signing payload — it's verified separately against the commitmentRoot).

- Pre-flight byte-for-byte proof (via standalone Python script using `nacl.signing.SigningKey` + `cbor2`): (a) imported the initiator's Ed25519 secret key `0x10*32` → constructed `SigningKey(sk)` → verified the derived public key matches `initiatorEd25519PubHex="5c9c6df261c9cb840475776aaefcd944b405328fab28f9b3a95ef40490d3de84"` byte-for-byte. (b) decoded the vector's `encodedHex` (598 hex chars / 299 bytes) via `cbor2.loads()` → extracted the 11 fields as a dict (cbor2 returns a Python dict for CBOR maps by default). (c) reconstructed the 9-field signing payload via `canonical_cbor_encode({1..9})` prefixed by `b"SHARENET/CIRCUIT/DESTROY/1"` → verified the Ed25519 signature via `VerifyKey(initEd25519Pub).verify(payload, sig)` → SUCCESS. (d) confirmed the routeId from the wire object matches `"route:" + bytes(commitmentRoot).hex()` (deterministic from the commitment root). (e) round-tripped: signed a fresh destroy via `sign_circuit_destroy(...)` → encoded via `encode_circuit_destroy(...)` → decoded via `decode_circuit_destroy(...)` → verified via `verify_circuit_destroy(...)` → all succeeded. Proves `nacl.signing`'s Ed25519 + `cbor2`'s canonical CBOR are byte-compatible with the TS `@noble/ed25519` + `cbor` package.

- Edit 1 (dispatch branch, lines 421-422): added `elif vid.startswith("V-CIRCUIT-DESTROY-"): return verify_circuit_destroy_vector(data)` AFTER the `V-CIRCUIT-GATEWAY-TEMPLATE-` branch + BEFORE the generic `V-CIRCUIT-` branch (mirrors the TS runner's dispatch order at lines 2434-2438). The order matters: `V-CIRCUIT-DESTROY-` is a strict prefix of `V-CIRCUIT-`, so the more-specific branch MUST come first to avoid the generic `V-CIRCUIT-` branch shadowing it.

- Edit 2 (constants, lines 3629-3656): added `CIRCUIT_DESTROY_DOMAIN = b"SHARENET/CIRCUIT/DESTROY/1"` (matches TS reference `destroy.ts:37`) + the 2 role constants (`DESTROYER_ROLE_INITIATOR=0x01`, `DESTROYER_ROLE_GATEWAY=0x02`) + the 5 reason constants (`DESTROY_REASON_OPERATOR_INITIATED=0x01` through `DESTROY_REASON_PROTOCOL_VIOLATION=0x05`) + the 11 CBOR integer key constants (`CD_KEY_CIRCUIT_ID=1` through `CD_KEY_SIGNATURE=11`). The integer-keyed map matches the TS reference `CD_KEY_*` constants at `destroy.ts:88-98`.

- Edit 3 (`circuit_destroy_signing_payload`, lines 3659-3710): implemented the 9-field canonical CBOR map (keys 1..9) under domain `SHARENET/CIRCUIT/DESTROY/1`. Added type-validation guards (destroyerRole / destroyReason / issuedAt / expiry must be ints not bools; routeId / destroyerNodeId must be strings) to surface programmer errors before encoding — mirroring the TS reference's implicit type-narrowing via TypeScript signatures. The payload is `domain || canonicalCBOR(map{1..9})`.

- Edit 4 (`sign_circuit_destroy`, lines 3713-3757): implemented the 9-arg signature (circuitId, commitmentRoot, destroyerNodeId, destroyerRole, destroyReason, issuedAt, expiry, destroyerEd25519SecretKey, destroyerEd25519PublicKey). Body: (1) derive routeId via `"route:" + bytes(commitment_root).hex()` (mirrors TS `toHex(commitmentRoot)`); (2) generate fresh 16-byte destroyNonce via `os.urandom(16)` (mirrors TS `randomBytes(16)`); (3) construct the 9-field signing payload + sign via `nacl.signing.SigningKey(destroyer_ed25519_secret_key).sign(payload).signature` (mirrors TS `signMessage(secretKey, payload)` — detached signature, 64 bytes); (4) return the 11-field wire dict.

- Edit 5 (`verify_circuit_destroy`, lines 3760-3823): implemented the 1-arg `verify(destroy)` function with the 3-step verify order mirroring the TS reference at `destroy.ts:218-250`: (1) role check (`destroyerRole ∈ {0x01, 0x02}` → `"invalid destroyerRole: X"`); (2) routeId derivation check (`"route:" + hex(commitmentRoot)` → `"routeId mismatch: expected \"X\", got \"Y\""`); (3) Ed25519 signature verify via `nacl.signing.VerifyKey(pub).verify(payload, sig)` (raises `nacl.exceptions.BadSignatureError` on failure → mapped to `"destroyer signature invalid (forged or tampered destroy)"`). The verify order matters: step 1 (role) and step 2 (routeId) fail BEFORE the signature check — so the `invalid-role-rejected` + `wrong-routeId-rejected` cases return their distinct reason strings, NOT the generic `"signature invalid"` reason.

- Edit 6 (`encode_circuit_destroy`, lines 3826-3844): implemented the 11-field canonical CBOR map (keys 1..11). All bstr fields are normalized via `bytes(...)` to ensure the encoded form is identical regardless of whether the caller passed `bytes` or `bytearray`.

- Edit 7 (`decode_circuit_destroy`, lines 3847-3935): implemented the strict decode with all 11 field validations. The `get_bstr(key, name, length)` helper validates type + exact length for the byte-string fields (circuitId/commitmentRoot=32 bytes; destroyNonce=16 bytes; destroyerEd25519PublicKey=32 bytes; signature=64 bytes). The routeId / destroyerNodeId fields must be strings. The destroyerRole / destroyReason / issuedAt / expiry fields must be integers (NOT booleans — `isinstance(x, bool)` check rejects `True`/`False` since `bool` is a subclass of `int` in Python, mirroring the TS `Number.isInteger()` check).

- Edit 8 (`verify_circuit_destroy_vector`, lines 3938-4144): implemented the dispatcher handling the 8 cases. The shared-input parsing extracts `circuitIdHex`, `commitmentRootHex`, `expiry`, `referenceNow`, `initiatorEd25519SecretKeyHex`, `initiatorEd25519PubHex`. The `gatewayEd25519SecretKeyHex` + `gatewayEd25519PubHex` are committed in the vector but unused by the 8 cases (the current vector exercises only the INITIATOR-signed destroy — a future vector may add a gateway-signed destroy case). The case dispatcher handles the 8 cases: (1) `sign-destroy` — asserts `destroyerNodeId`, `destroyerRole`, `destroyReason`, `routeId` (exact match) + `encodedLen=299` (LENGTH not exact bytes — the destroyNonce is random per sign call, matching the TS runner at `ts-vector-runner.ts:1638-1643`); (2) `decode-destroy` — decode the input's `encodedHex` (299 bytes) → ok; (3) `verify-destroy` — verify with the original destroy → ok; (4) `wrong-signer-rejected` — replace `destroyerNodeId` with `"wrong-node-id"` → `"signature invalid"` (signature was over ORIGINAL destroyerNodeId, so step-3 check fails); (5) `tampered-reason-rejected` — replace `destroyReason` with `0x99` → `"signature invalid"`; (6) `tampered-nonce-rejected` — replace `destroyNonce` with `0xFF*16` → `"signature invalid"`; (7) `invalid-role-rejected` — set `destroyerRole=0x03` → `"invalid destroyerRole"` (step-1 check fails BEFORE the signature check); (8) `wrong-routeId-rejected` — replace `routeId` with `"route:wrong"` → `"routeId mismatch"` (step-2 check fails BEFORE the signature check — routeId is NOT in the signing payload).

- Verification:
  * `python3 conformance/runners/py_vector_verifier.py 2>&1 | tail -5` → `Passed: 40/40, Failed: 0` (exit 0). UP from 39/39 (added `V-CIRCUIT-DESTROY-001` as the 40th vector). All 39 previously-passing vectors still pass — ZERO regressions.
  * `python3 conformance/runners/py_vector_verifier.py 2>&1 | grep V-CIRCUIT-DESTROY` → `[PASS] V-CIRCUIT-DESTROY-001`.
  * `python3 conformance/runners/py_vector_verifier.py 2>&1 | grep -c '^\s*\[PASS\]'` → 40; `grep -c '^\s*\[FAIL\]'` → 0.
  * Standalone driver: `python3 -c "import py_vector_verifier as v; r=v.verify_circuit_destroy_vector(json.load(open('conformance/vectors/V-CIRCUIT-DESTROY-001.json'))); print(r['passed'], r['actual'])"` → `True "8 circuit-destroy cases match"`. All 8 cases pass independently.
  * `python3 -m py_compile conformance/runners/py_vector_verifier.py` → OK (no syntax errors).
  * `cd /home/z/my-project && bun run test:unit 2>&1 | tail -6` → `453 pass / 0 fail / 1575 expect() calls across 29 files in 3.27s`. UP from 452 pass / 1 fail (task 13 baseline). The previously-failing R-007 completeness test (`TS runner and Python runner dispatch the same set of vector prefixes`) now PASSES — the `V-CIRCUIT-DESTROY-` prefix is in BOTH runners.
  * Specifically the R-007 completeness suite: `bun test tests/r007-completeness.test.ts 2>&1 | tail -5` → `17 pass / 0 fail`. All 17 R-007 tests pass — including the previously-failing `TS runner and Python runner dispatch the same set of vector prefixes` (the `onlyTs` array is now `[]` — the Python runner's `V-CIRCUIT-DESTROY-` dispatch branch matches the TS runner's).
  * `cd /home/z/my-project && bun run lint 2>&1; echo "LINT_EXIT=$?"` → exit code 0 (clean). No lint warnings introduced.
  * Pre-flight byte-for-byte proof (standalone Python script): decoded the vector's `encodedHex` (299 bytes) via `cbor2.loads()` → extracted the 11 fields. Reconstructed the 9-field signing payload via `canonical_cbor_encode({1..9})` prefixed by `b"SHARENET/CIRCUIT/DESTROY/1"`. Verified the Ed25519 signature via `nacl.signing.VerifyKey(initEd25519Pub).verify(payload, sig)` → SUCCESS. Confirms cross-implementation byte compatibility between the Python `nacl.signing` + `cbor2` stack and the TS `@noble/ed25519` + `cbor` package stack for the CircuitDestroy wire object.

- Independence preserved: AST-walked the file imports — only stdlib + `blake3`, `cbor2`, `nacl.signing.SigningKey`/`nacl.signing.VerifyKey`, `nacl.exceptions.BadSignatureError`, `cryptography.hazmat.primitives.asymmetric.x25519.X25519PrivateKey`/`X25519PublicKey`, `cryptography.hazmat.primitives.serialization`, `cryptography.hazmat.primitives.ciphers.aead.ChaCha20Poly1305`, `cryptography.exceptions.InvalidTag`. ZERO imports from `reference/` or any TS-side module. The new `circuit_destroy_signing_payload` / `sign_circuit_destroy` / `verify_circuit_destroy` / `encode_circuit_destroy` / `decode_circuit_destroy` are reproduced independently from the TS reference `reference/circuit/destroy.ts` using `nacl.signing` (Ed25519) + `cbor2.dumps(..., canonical=True)` (canonical CBOR) + `os.urandom(16)` (random nonce). The pre-flight byte-for-byte proof (decode the vector's encodedHex + verify the signature) demonstrates cross-implementation conformance for the Ed25519 + canonical CBOR stack.

Stage Summary:
- Python conformance runner: 40/40 vectors pass (UP from 39/39 — added `V-CIRCUIT-DESTROY-001` as the 40th vector). `V-CIRCUIT-DESTROY-001` passes all 8 cases of the authenticated circuit teardown structure (sign-destroy / decode-destroy / verify-destroy / wrong-signer-rejected / tampered-reason-rejected / tampered-nonce-rejected / invalid-role-rejected / wrong-routeId-rejected). The 3-step verify order (role → routeId → signature) ensures the distinct reject reasons are surfaced: invalid-role returns `"invalid destroyerRole"` (step 1), wrong-routeId returns `"routeId mismatch"` (step 2), and the 3 tampering cases (wrong-signer / tampered-reason / tampered-nonce) all return `"signature invalid"` (step 3 — because the signature was over the ORIGINAL fields).
- The R-009 Stage 3 CircuitDestroy wire object is now Python-verifier-complete: the wire object carries `circuitId` (32 bytes) + `commitmentRoot` (32 bytes) + `routeId` (text, derived as `"route:" + hex(commitmentRoot)`) + `destroyerNodeId` (text) + `destroyerRole` (uint — 0x01 INITIATOR or 0x02 GATEWAY) + `destroyReason` (uint — enumerated reason code) + `destroyNonce` (16 bytes, fresh per sign) + `issuedAt` (uint) + `expiry` (uint) + `destroyerEd25519PublicKey` (32 bytes) + `signature` (64 bytes Ed25519). The signing payload is `domain || canonicalCBOR(map{1..9})` (excludes the pubkey + signature — these are carried in the wire but not signed). The signature binds the destroyer identity (destroyerNodeId + destroyerRole) + the destroy reason + the replay nonce + the circuit lifetime — so any tampering with these fields invalidates the signature. The signature is portable: anyone with the `destroyerEd25519PublicKey` (carried in the wire) can verify it without any WeakSet or in-process proof.
- The defense-in-depth chain — role validation → routeId derivation → Ed25519 signature — is now fully mirrored in the independent Python verifier. The 8 cases prove the fail-closed posture on every authentication dimension: signature tampering (wrong-signer / tampered-reason / tampered-nonce), invalid role (only INITIATOR + GATEWAY may originate; relays cannot), routeId integrity (routeId must match the commitment root). Authorization checks (is the destroyer the actual initiator/gateway of THIS circuit?) are performed separately by the caller using the circuit context — the portable verifier authenticates the destroyer's identity but does not authorize the destroy against the circuit's actual participants (that's the caller's job using the route's initiatorNodeId for INITIATOR or the portable proof chain for GATEWAY).
- Cross-implementation conformance (TS ↔ Python) is preserved for the new vector: both runners independently reproduce the Ed25519 + canonical CBOR byte stream from the frozen `V-CIRCUIT-DESTROY-001.json` vector file. The spec↔registry↔manifest↔TS↔Python conformance chain for the CircuitDestroy wire object is now closed.
- The R-007 completeness gate is now SATISFIED: TS + Python dispatch the same set of vector prefixes (including the new `V-CIRCUIT-DESTROY-` prefix), and every manifest vector ID is dispatched by BOTH runners. The 453/453 unit-test pass count (UP from 452 pass / 1 fail in task 13) confirms ZERO regressions — the previously-failing R-007 test now passes.
---
Task ID: 15-required-revocation-tests
Agent: general-purpose (subagent)
Task: Update all R-009 test files for the new REQUIRED `revocationStore` parameter on `processCircuitWireFrame(circuit, hopIndex, wireBytes, revocationStore, now)`. The signature change in `reference/circuit/forwarding.ts:249-266` made `revocationStore` REQUIRED (was optional in task 13). Every `undefined, NOW` 4th-arg call left by task 13 must now pass a real `InMemoryCircuitRevocationStore()`.

Work Log:
- Read context: worklog.md last 3 sections (13-stage3-test-fixes, 14-python-destroy-verifier, plus the implicit 11-terminal-proof-completion chain) — confirmed task 13 had appended `, undefined, NOW` to 36 `processCircuitWireFrame` call sites (25 in `r009-circuit-frame.test.ts` + 11 in `r009-return-template.test.ts` + 1 in `r009-multiprocess.test.ts`) and that the 4th-arg `undefined` was a stopgap now superseded.

- Audited call sites via Grep for `processCircuitWireFrame.*undefined` across the repo (excluding `worklog.md` historical text): 39 hits total — 25 in `r009-circuit-frame.test.ts`, 11 in `r009-return-template.test.ts`, 2 in `r009-destroy.test.ts` (an expiry test + a non-expiry test, both new in task 14's destroy work), and 1 inside the `relayScript` template literal in `r009-multiprocess.test.ts`. `r008-durable-integration.test.ts` uses ONLY the lower-level `processCircuitFrame(circuit, hopIndex, frameSequence, direction, ciphertext)` (5-arg signature WITHOUT `revocationStore`/`now`) — UNAFFECTED, skipped per task brief. `conformance/runners/ts-vector-runner.ts` has ZERO `processCircuitWireFrame` references — UNAFFECTED, skipped per task brief.

- Confirmed `InMemoryCircuitRevocationStore` exists at `reference/circuit/replay-stores.ts:451` (already exported; implements `CircuitRevocationStore` interface with `isRevoked(cid, cr) → Promise<boolean>` + `revoke(cid, cr, destroyerNodeId, destroyerRole, destroyReason, destroyNonce) → Promise<boolean>`). No new code needed in the reference module.

- Edit 1 (`tests/r009-circuit-frame.test.ts:54`): extended the existing single-import line `import { InMemoryCircuitSequenceFloorStore } from "@reference/circuit/replay-stores";` → added `, InMemoryCircuitRevocationStore`. Then bulk-replaced 25 occurrences of `, undefined, NOW)` → `, new InMemoryCircuitRevocationStore(), NOW)` via `replace_all: true`. Verified post-edit: Grep `processCircuitWireFrame.*new InMemoryCircuitRevocationStore` returns 25 matches; `processCircuitWireFrame.*undefined.*NOW` returns 0.

- Edit 2 (`tests/r009-return-template.test.ts:46`): extended the existing import line `import { InMemoryCircuitSequenceFloorStore, InMemoryGatewayAuthorizationReplayStore } from "@reference/circuit/replay-stores";` → appended `, InMemoryCircuitRevocationStore`. Then bulk-replaced 11 occurrences of `, undefined, NOW)` → `, new InMemoryCircuitRevocationStore(), NOW)` via `replace_all: true`. Verified: 11 matches with the new store, 0 with `undefined`.

- Edit 3 (`tests/r009-multiprocess.test.ts:237-289`): the relay script is a template literal evaluated by `bun -e`. Added a new `require` line at line 239 inside the template: `const { InMemoryCircuitRevocationStore } = require("${join(process.cwd(), "reference/circuit/replay-stores.ts").replace(/\\/g, "\\\\")}");` — mirrors the existing require pattern for `processCircuitWireFrame` (line 238) and `InMemoryGatewayAuthorizationReplayStore` (line 156). Then replaced the inline call `processCircuitWireFrame(activeCircuit, data.hopIndex, wireBytes, undefined, data.now)` → `processCircuitWireFrame(activeCircuit, data.hopIndex, wireBytes, new InMemoryCircuitRevocationStore(), data.now)`. Updated the in-template comment (single-quoted per the prior task's note — backticks would prematurely close the outer template literal) to explain the fresh `InMemoryCircuitRevocationStore` exercises the relay's revocation check with no durable record.

- Edit 4 (`tests/r009-destroy.test.ts:232-236 + 253-254`): the file already imported `InMemoryCircuitRevocationStore` (line 49, added in task 14). Two `undefined` call sites needed replacing — (a) the expiry test at line 233 (`now = route.branded.expiry + 1`): the production path performs a DURABLE revocation (writes a `CIRCUIT_EXPIRED` record into the store) before returning — calling `.revoke()` on `undefined` would throw `TypeError: Cannot read properties of undefined`. Replaced with a fresh `const revocationStore = new InMemoryCircuitRevocationStore();` local + passed it as the 4th arg. (b) the non-expiry test at line 250: the production path calls `.isRevoked()` on the store — same `TypeError` risk on `undefined`. Replaced with an inline `new InMemoryCircuitRevocationStore()`. The third call site (line 285 — "revoked circuit → frame REJECTED") already passed the `DurableSqliteCircuitRevocationStore` from the describe-level `beforeAll`, so it was already correct and unchanged. The new comments document WHY the real store matters for the expiry path (durable terminal-state transition, not a local rejection).

- Verified the 2 unaffected files per task brief: `r008-durable-integration.test.ts` uses only `processCircuitFrame` (12 calls — the 5-arg lower-level API, no `revocationStore` param); `conformance/runners/ts-vector-runner.ts` has 0 `processCircuitWireFrame` references. Both correctly skipped.

- Verification:
  * `cd /home/z/my-project && bun test tests/r009-circuit-frame.test.ts tests/r009-return-template.test.ts tests/r009-multiprocess.test.ts tests/r009-destroy.test.ts 2>&1 | tail -5` → `96 pass / 0 fail / 352 expect() calls` across 4 files in 1.97s. All 4 affected files pass with the new required `revocationStore` arg.
  * `cd /home/z/my-project && bun run test:unit 2>&1 | tail -6` → `468 pass / 0 fail / 1605 expect() calls across 30 files in 3.19s`. UP from 453 pass (task 14 baseline). ZERO regressions across the entire unit suite. The +15 test delta comes from a previously-unrun test file `tests/r009-durable-auth-replay.test.ts` (verified to NOT call `processCircuitWireFrame` — it tests the durable auth-replay stores directly, so it is unrelated to this signature change) — it was simply picked up by `test:unit`'s globbing now.
  * `cd /home/z/my-project && bun run lint 2>&1; echo "LINT_EXIT=$?"` → exit code 0 (clean). The 38 call-site edits + the 2 import-line extensions + the multiprocess template-literal `require` addition introduce no lint warnings.

- Independence preserved: the test changes use ONLY existing identifiers (`InMemoryCircuitRevocationStore` — already exported from `reference/circuit/replay-stores.ts:451` since task 14; `NOW`, `new`, `require`, `processCircuitWireFrame`, `InMemoryCircuitSequenceFloorStore`, `InMemoryGatewayAuthorizationReplayStore`). No new modules, no new helpers, no new dependencies. The multiprocess template-literal change reuses the existing `join(process.cwd(), "reference/circuit/replay-stores.ts").replace(/\\/g, "\\\\")` path-construction pattern (already used 2 lines above for the gateway script's `InMemoryGatewayAuthorizationReplayStore` require). No source-file changes (no changes under `reference/` or `conformance/`) — purely test-side.

Stage Summary:
- All 39 `processCircuitWireFrame` call sites that previously passed `undefined` for `revocationStore` (25 + 11 + 2 + 1 across the 4 affected test files) now pass a real `InMemoryCircuitRevocationStore` instance. The signature `processCircuitWireFrame(circuit, hopIndex, wireBytes, revocationStore, now)` is now REQUIRED as intended — no `undefined`-arg workarounds remain in the test suite.
- The expiry-path tests in `r009-destroy.test.ts` now exercise the FULL durable terminal-state transition: when `now >= circuit.expiry`, the production path writes a `CIRCUIT_EXPIRED` revocation record into the store (with destroyerNodeId="system", destroyerRole=INITIATOR, destroyReason=CIRCUIT_EXPIRED) and zeroizes the circuit keys — the test verifies this completes without throwing and returns the expected `"expired"` rejection reason. A future test could additionally assert `await revocationStore.isRevoked(circuit.circuitId, circuit.commitmentRoot) === true` post-expiry to prove the durable record was actually written, but the existing assertion (`result.ok === false` + `reason contains "expired"`) is sufficient for the immediate "tests pass" goal.
- The multi-process test correctly threads the `InMemoryCircuitRevocationStore` into the child Bun process: parent imports it (not strictly needed — the child constructs its own) + the child's relayScript template literal now `require()`-imports `InMemoryCircuitRevocationStore` from `reference/circuit/replay-stores.ts` + constructs a fresh instance per frame. The child's revocation check (step 0b) now runs against a real (empty) store rather than crashing on `undefined.isRevoked()`.
- The 2 unaffected files (`r008-durable-integration.test.ts` + `conformance/runners/ts-vector-runner.ts`) correctly remain untouched: the former uses the lower-level `processCircuitFrame` API (no `revocationStore`/`now` params), the latter has zero `processCircuitWireFrame` calls. No false-positive edits.
- The `processCircuitWireFrame.length === 5` assertion at `tests/r009-circuit-frame.test.ts:985` (set in task 13) remains accurate — the signature arity did not change (was 5 in task 13, still 5 now; the change is that the 4th param's TYPE became non-nullable, not that a param was added or removed).

---
Task ID: R009-S3-reaudit
Agent: orchestrator
Task: Re-audit commit 6936831 and fix the four remaining teardown gaps before propagation.

Work Log:
- Read previous worklog + git log: confirmed at commit 6936831 (R-009 Stage 3: processCircuitDestroy + required revocationStore + zeroize + durable expiry).
- Read the four audit-flagged files: reference/circuit/destroy.ts, reference/circuit/forwarding.ts, reference/circuit/replay-stores.ts, reference/circuit/circuit.ts (ActiveCircuit type), tests/r009-destroy.test.ts, adr/0022-circuit-destroy-and-teardown-semantics.md, spec/08-circuits.md §6/§6.5a/§4.7, conformance/vectors/V-CIRCUIT-DESTROY-001.json + conformance/runners/ts-vector-runner.ts (verifyCircuitDestroyVector).
- Confirmed verifyNodeIdBinding already exists at reference/identity/keys.ts:110 (BLAKE3-256 over the public key, constant-time compare).
- Gap #1 (identity bypass): processCircuitDestroy compared destroyerNodeId === expectedInitiatorNodeId WITHOUT verifying that destroyerEd25519PublicKey derives destroyerNodeId. An attacker who learns a legitimate initiator's public NodeId string could sign a forged destroy with their own keypair (putting their own pubkey in destroyerEd25519PublicKey) while setting destroyerNodeId to the legitimate value; the signature would verify against the attacker's pubkey and the role check would pass.
- Gap #2 (expiry not fail-closed): processCircuitWireFrame expiry path called `await revocationStore.revoke(...)` and IGNORED the boolean return. If the durable write failed, the path still returned "circuit expired" — falsely claiming durable expiry — then zeroized keys.
- Gap #3 (zeroize not owned by processCircuitDestroy): the canonical teardown path left a comment "The caller is responsible for calling zeroizeCircuit(circuit) from forwarding.ts. We don't import it here to avoid a circular dependency."
- Gap #4 (lifecycle semantics): spec §6 said CIRCUIT_EXPIRED is an event → CIRCUIT_REVOKED, but did not explicitly state the durable tombstone is the AUTHORITATIVE terminal state (ACTIVE = no tombstone; REVOKED = tombstone exists).

Fixes applied:
- Fix #1: Added verifyNodeIdBinding(destroyerNodeId, destroyerEd25519PublicKey) check as step 4a in processCircuitDestroy (destroy.ts), BEFORE the role authorization (step 4b). Returns "identity binding failed — forged destroy rejected" on mismatch. CRITICAL design decision: added the check ONLY to processCircuitDestroy (the production path), NOT to the portable verifyCircuitDestroy — because the frozen V-CIRCUIT-DESTROY-001 vectors use a placeholder NodeId "initiator-node-id" that is not a real BLAKE3-derived base32 NodeId. This surgical placement preserves all 8 frozen conformance vectors while closing the bypass in the only path that has circuit context.
- Fix #2: Made the expiry path in processCircuitWireFrame (forwarding.ts) check the return value of revocationStore.revoke(). If false (persistence failure): return ok:false with reason "circuit expired BUT durable revocation persistence FAILED (fail-closed) ... tombstone NOT persisted, keys retained for retry" and do NOT zeroize. Only if revoke() returns true: zeroize + return "circuit expired: ... (durably revoked)". Imported DESTROYER_ROLE_INITIATOR + DESTROY_REASON_CIRCUIT_EXPIRED from ./destroy.ts (replacing inline magic numbers 0x01/0x02).
- Fix #3: Created reference/circuit/zeroize.ts (new file) housing zeroizeCircuit(), so that BOTH destroy.ts (processCircuitDestroy) and forwarding.ts (processCircuitWireFrame) can import it WITHOUT a circular dependency (destroy.ts ↔ forwarding.ts). forwarding.ts re-exports zeroizeCircuit for backwards compatibility. processCircuitDestroy now calls zeroizeCircuit(circuit) itself in BOTH the fresh-destroy path (step 8, after the tombstone is confirmed persisted) AND the idempotent path (step 5, after isRevoked). Removed the "caller is responsible" comment.
- Fix #4: Documented the authoritative tombstone semantics in ADR-0022 §1 (state machine), §3 (authorization — two-layer model), §7 (fail-closed expiry), §9 (zeroize owned by processCircuitDestroy) and spec/08 §6.3 (authoritative terminal state), §6.5a (identity authorization two-layered), §4.7 (fail-closed expiry). Explicit statement: ACTIVE ≡ no tombstone; REVOKED ≡ tombstone exists; a local transient cleanup state MUST NEVER contradict the durable tombstone; no second competing state machine.

Adversarial tests added (tests/r009-destroy.test.ts, +7 tests, +358 lines):
- Fix #1 (identity bypass): "valid initiator destroy → ACCEPT", "attacker key + legitimate initiator NodeId → REJECT (identity binding fails)", "attacker key + legitimate gateway NodeId → REJECT", "valid gateway destroy → ACCEPT". The attacker tests generate a fresh attacker keypair, sign with the attacker's secret key, claim the legitimate NodeId — and prove the forged destroy is rejected + the circuit is NOT revoked + the destroy nonce is NOT consumed (retryable).
- Fix #2 (fail-closed expiry): "revocation persistence failure → frame rejected, fail-closed, keys NOT zeroized" using a FailingCircuitRevocationStore test double (revoke() always returns false). Asserts: reason contains "persistence FAILED" + "fail-closed", reason does NOT contain "durably revoked", keys are byte-identical to the pre-expiry snapshot (retained for retry), floor does not advance.
- Fix #3 (zeroize ownership): "processCircuitDestroy succeeds → all key material zeroized WITHOUT caller-side zeroize" — calls processCircuitDestroy and asserts all hops' forwardingKey/returnKey/relayX25519PublicKey + initiatorX25519SecretKey + noncePrefix are all zeros, WITHOUT the test ever calling zeroizeCircuit(). commitmentRoot + circuitId retained. Plus "idempotent destroy → keys still zeroized" proving the idempotent path also zeroizes (re-mangles a key then re-destroys).
- IMPORTANT test-hygiene fix: the existing tests (inherited from 6936831) used `const initiatorKp = route.kps[0]!` (hop 0, a RELAY's keypair) while claiming `route.initiator.nodeId` (the initiator's NodeId). These NEVER matched — they inadvertently exercised the exact identity bypass the audit describes (signing with the wrong key but claiming the right NodeId). The old code without verifyNodeIdBinding accepted them; my new check correctly rejects them. Fixed all 7 occurrences to use `route.initiator` (the real initiator keypair, whose publicKey derives route.initiator.nodeId). This is the core proof that the bypass was real + is now closed.

Verification (all five required suites):
- Unit tests: 483 pass / 0 fail (was 476; +7 adversarial tests). 1684 expect() calls across 30 files in 3.76s.
- Architecture tests: 24/24 pass (incl. #21 + #23 layer-separation: reference/ has zero imports from @/lib/db + src/; the new reference/circuit/zeroize.ts only imports the ActiveCircuit TYPE from ./circuit.ts).
- TS conformance vectors: 40/40 pass (incl. V-CIRCUIT-DESTROY-001 — the 8 destroy vectors pass because verifyNodeIdBinding was added ONLY to processCircuitDestroy, not to the portable verifyCircuitDestroy).
- Python conformance vectors: 40/40 pass.
- Lint: clean (exit 0).

Stage Summary:
- All four teardown gaps closed. No propagation or recovery work started (per the audit's stop condition).
- The identity authorization bypass is closed via a two-layer model (Layer 1: identity binding verifyNodeIdBinding; Layer 2: role authorization) enforced ONLY in the production path processCircuitDestroy — preserving the portable verifier's structure-only contract + all frozen conformance vectors.
- Expiry is fail-closed: a persistence failure returns an explicit "persistence FAILED" reason, does NOT claim durable expiry, does NOT zeroize keys (retained for retry).
